...
# Function to Execute Queries & Return DataFrame
def fetch_data(query):
    conn = db_config() 
    df = pd.read_sql(query, conn)
    conn.close()
...
# 🎯 Query 6: Top 10 Customers by Total Order Value
query6 = """
    SELECT c.name, ROUND(SUM(o.total_amount)) AS total_order_value
    FROM customers c 
    JOIN orders o ON c.customer_id = o.customer_id
    GROUP BY c.customer_id, c.name
    ORDER BY total_order_value DESC
...
sns.barplot(x=df6['total_order_value'], y=df6['name'], palette="Blues_r")
...
plt.figure(figsize=(10, 6))
sns.barplot(x=df9['total_deliveries'], y=df9['name'], palette="Greens_r")
plt.xlabel("Total Deliveries")
plt.ylabel("Delivery Person")
plt.title("Deliveries per Delivery Person")
plt.show()

# 🎯 Query 12: Most Popular Restaurants by Order Frequency
query12 = """
    SELECT r.name, COUNT(o.order_id) AS order_frequency
    FROM restaurants r
    JOIN orders o ON r.restaurant_id = o.restaurant_id
    GROUP BY r.restaurant_id, r.name
    ORDER BY order_frequency DESC;
...
plt.figure(figsize=(10, 6))
sns.barplot(x=df12['order_frequency'], y=df12['name'], palette="Oranges_r")
plt.xlabel("Order Frequency")
plt.ylabel("Restaurant")
plt.title("Most Popular Restaurants by Order Frequency")
plt.show()

# 🎯 Query 14: Days with the Most Orders
query14 = """
    SELECT DAYOFWEEK(order_date) AS day_of_week, COUNT(*) AS order_count
    FROM orders
    GROUP BY day_of_week
...
plt.figure(figsize=(10, 6))
sns.barplot(x=df14['day_of_week'], y=df14['order_count'], palette="coolwarm")
